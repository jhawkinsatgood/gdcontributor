...
function syncWWW()
{
    ...
    # Create any directories that exist in the source, but not in the
    # target.
    ...
    # Copy any files that are newer than the root. The calling code has to
    # set the modification time of the root so that there is no wasteful
    # copying.
